...
r = 0.005  # assumption
sigma, lamb, mu, delta = [0.124, 1.393, -0.137, 0.000]  # from calibration
...
def M76_generate_paths(S0, T, r, sigma, lamb, mu, delta, M, I):
    ...
    for t in range(1, M + 1, 1):
        if disc == 1:
            S[t] = S[t - 1] * ((1 + (r - rj) * dt) + sigma * math.sqrt(dt) * rand1[t]
                               + (np.exp(mu + delta * rand2[t]) - 1) * rand3[t])
        else:
            S[t] = S[t - 1] * (np.exp((r - rj - 0.5 * sigma ** 2) * dt + sigma * math.sqrt(dt) * rand1[t])
                               + (np.exp(mu + delta * rand2[t]) - 1) * rand3[t])
    return S
...
if __name__ == '__main__':
    ...
    plt.grid()
    plt.plot(strikes, values[0,],'b',label = 'INT', lw = 1.5)
    plt.plot(strikes, values[1,],'ro',label = 'FFT', lw = 1.5)
    plt.plot(strikes, values[2,],'g^',label = 'MCS', lw = 1.5)

...
def rolling_statistics(data):
    ...
    mr = pd.Series(data['returns']).rolling(window=252).mean() * 252
    mr.plot()
    plt.grid(True)
    ...
    plt.subplot(312)
    vo = pd.Series(data['returns']).rolling(window=252).std() * math.sqrt(252)
    vo.plot()
    plt.grid(True)
    ...
    co = pd.Series(mr).rolling(window=252).corr(vo)
    co.plot()
    plt.grid(True)

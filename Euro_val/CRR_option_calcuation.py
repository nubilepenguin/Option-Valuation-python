...
def CRR_option_value(S0, K, T, r, sigma, otype, M = 4):
    ...
    # Inner Values
    if otype == 'call':
        V = np.maximum(S - K, 0) # inner value for European call option
    else:
        V = np.maximum(K - S, 0) # inner value for European put option
...
CRR_option_value(S0, K, T, r, sigma, otype='call',M=4)
...
def plot_convergence(mmin, mmax, step_size):
    ...
    plt.xlim(0, mmax)
    plt.show()
plot_convergence(4,1000,20)
plot_convergence(4,1000,25)
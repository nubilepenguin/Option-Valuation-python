...
def inner_value(S, otype):
    ''' Inner value functions for American put option and short condor spread option with American exercise.
    Parameters
    ==========
    otype: int
        option type
    ...
    if otype == 1:
        ...
        return np.minimum(40.0, np.maximum(90. - S, 0) + np.maximum(S - 110.,0))
    else:
        raise ValueError('Option type not known.')
...
def CRR_option_valuation(otype, M = 500):
    ...
    for i in range(M - 1, -1, -1):
        ...
        V[0 : M - z, i] = np.where(h[0 : M - z, i] > C[0:M - z, i],
                                   h[0:M - z, i], C[0:M - z, i])
        ex[0 : M - z, i] = np.where(h[0 : M - z, i] > C[0:M - z, i],
                                    0, 1)
        z += 1
...
print(CRR_option_valuation(otype=1,M=500))
...
from gen_frame.BCC_option_valuation import H93_call_value
from MCS.H93_european_mcs import SRD_generate_paths
from MCS.CIR_zcb_valuation_gen import B
from time import time

# 'True' American Options Prices by Monte Carlo
# from MS (2009), table 3

benchmarks = np.array(((0.0001, 1.0438, 9.9950, 0.0346, 1.7379, 9.9823, 0.2040, 2.3951, 9.9726),  # panel
                       (0.0619, 2.1306, 10.0386, 0.5303, 3.4173, 10.4271, 1.1824, 4.4249, 11.0224),  # panel2
                       (0.0592, 2.1138, 10.0372, 0.4950, 3.3478, 10.3825, 1.0752, 4.2732, 10.8964),  # panel3
                       (0.0787, 2.1277, 10.0198, 0.6012, 3.4089, 10.2512, 1.2896, 4.4103, 10.6988)))  # panel4
...
para = np.array(((0.01, 1.5, 0.15, 0.1),  # pannel 1
                 # (v0, kappa_v, sigma_v, rho)
                 (0.04, 0.75, 0.3, 0.1),  # pannel 2
                 (0.04, 1.50, 0.3, 0.1),  # pannel 3
                 (0.04, 1.5, 0.15, -0.5)))  # pannel 4
theta_v = 0.2  # long-term variance level
...
x_disc_list = ['Full Truncation', 'Partial Truncation', 'Truncation', 'Absorption',
               'Reflection', 'Higham-Mao', 'Simple Reflection']
    ...
    for run in range(runs):  # simulation runs
        for panel in range(4):  # panels
            if verbose:
                ...
                gc.collect()
                for T in t_list:  # time to maturity
                    # discount factor
                    ...
                    v = SRD_generate_paths(x_disc, v0, kappa_r, theta_r, sigma_r,
                    ...
                    for K in k_list:  # strikes
                        ...
                        for t in range(M - 1, 0, -1):
                            ...
                            if no_itm == 0:
                                ...
                                rel_V = (np.compress(itm, V[t + 1]) * np.compress(itm, df))
                        ...
                        # standard error
                        SE = np.std(y_cv) / math.sqrt(I)  # benchmark value
                        V0 = benchmarks[panel, z]
                        # LSM control variate
                        V0_CV = max(np.sum(y_cv) / I, h[0, 0])  # pure LSM
                        ...
                        z += 1  # option counter
                        if verbose:
                            print(tmpl_2 % (T, K, V0, V0_LSM, V0_CV, P0, P0_MCS, error, rel_error, PY1_acc, PY2_acc))
                        results = results.append(res, ignore_index=True)

    if write:
        d = str(datetime.now().replace(microsecond=0))
        d = d.replace('-', '_').replace(' ', '_').replace(':', '_')
        h5 = pd.HDFStore('mcs_american_%s.h5' % d, 'w')
        h5['results'] = results
        h5.close()

...
paths_list = [25000, 500000, 75000, 100000] # number of paths per valuation
s_disc_list = ['Log', 'Naive'] # Euler scheme: log vs. navie
...
def SRD_generate_paths(x_disc, x0, kappa, theta, sigma,
    ...
    for t in range(1, M + 1):
        ...
        if x_disc == 'Full Truncation':
            ...
            x[t] = np.maximum(0, x[t - 1] + kappa * (theta - x[t - 1]) * dt +
            ...
            x[t] = abs(xh[t - 1] + kappa * (theta - xh[t - 1]) * dt +
                      np.sqrt(xh[t - 1]) * sigma * ran[row] * sdt)
        elif x_disc == 'Absorption':
            xh[t] = (np.maximum(0, xh[t - 1]) + kappa * (theta - np.maximum(0, xh[t - 1])) * dt
                     + np.sqrt(np.maximum(0, xh[t - 1])) * sigma * ran[row] * sdt)
            x[t] = np.maximum(0, xh[t])
        else:
            print(x_disc)
...
if __name__ == '__main__':
                ...
                for T in t_list: #maturity list
                    ...
                    for K in k_list:
                        ...
                        res = pd.DataFrame({'timestamp': datetime.now(),
                                            ...
                                            'var_disc': x_disc, 'anti_paths': antipath,'moment_matching': momatch, 'panel': panel,
                                           ...
                                           index=[0,])
                        if verbose:
                            print(tmpl_2 % (T, K, V0, V0_MCS, error, rel_error, PY1_acc, PY2_acc))
    ...
    if write:
        ...
        h5 = pd.HDFStore('mcs_european_%s_%s.h5'
                         % (d[:10], d[11:]), 'w')
        h5['results'] = results
        h5.close()
